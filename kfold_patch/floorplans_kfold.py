...
def load_train_data(classes, dataset, normalize=True, buffer_size=400, base_dir='data', n_upsample=None,
    ...
    if fold is not None and k_fold is not None and k_fold > 0:
        ...
        for k in range(k_fold):
            if k != fold:
                train_datasets.append(load_dataset(os.path.join(base_dir, dataset + f'_fold_{k}'),
                                                   normalize, classes, n_upsample, reduction_ratio))
...
def create_dataset(datasets, type='default', input_dirs='./', output_dir='./', data_dir='annotations/hdd/', k_fold=0):
    ...
    if k_fold > 0:
        test_paths = []
        fold_paths = [[] for _ in range(k_fold)]
        for input_dir, dataset in zip(input_dirs, datasets):
            ...
            for k in range(k_fold):
                fold_paths[k] += open(os.path.join(input_dir, dataset + f'_fold_{k}.txt'), 'r').read().splitlines()

        random.shuffle(test_paths)
        for k in range(k_fold):
            random.shuffle(fold_paths[k])
        ...
        for k in range(k_fold):
            write_record(fold_paths[k], name=os.path.join(output_dir, dataset_name + f'_fold_{k}'), type=type, data_dir=data_dir)
    else:
        train_paths = []
        val_paths = []
        test_paths = []
