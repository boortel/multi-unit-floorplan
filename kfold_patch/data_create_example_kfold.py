...
def dataset_create(dataset_name, classes, heatmap_inds, data_source, data_dir, ann_dir, out_dir, recreate, augment, overlay, k_fold):
    ...
    if data_source == 'r3d':
        ...
        if not os.path.exists(out_dir) or not any([file.split('.')[-1] == 'tfrecords' for file in os.listdir(out_dir)]) or recreate:
            print('Creating tf records...')
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)
            create_dataset(data_source, type='default', input_dirs=ann_dir, output_dir=out_dir,
                           data_dir=os.path.dirname(data_dir), k_fold=k_fold)
        ...
        if not os.path.exists(out_dir) or not any([file.split('.')[-1] == 'tfrecords' for file in os.listdir(out_dir)]) or recreate:
            ...
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)
            create_dataset(data_source, type='cubicasa5k', input_dirs=ann_dir, output_dir=out_dir,
                           data_dir=os.path.dirname(data_dir), k_fold=k_fold)
        if overlay:
            # Create the overlay
        ...
        _ = split_dataset(data_source, input_file=input_image_name, data_dir=data_dir, output_dir=ann_dir,
                          filtering=cvc_select, subfolders_only=os.path.exists(ann_dir) and not recreate, k_fold=k_fold)
        ...
        if not os.path.exists(out_dir) or not any([file.split('.')[-1] == 'tfrecords' for file in os.listdir(out_dir)]) or recreate:
            ...
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)
            create_dataset(data_source, input_dirs=ann_dir, output_dir=out_dir,
                           data_dir=os.path.dirname(data_dir), k_fold=k_fold)
        ...
        print('Creating dataset splits...')
        _ = split_dataset(data_source, input_file=input_image_name, data_dir=data_dir, output_dir=ann_dir,
                          subfolders_only=os.path.exists(ann_dir) and not recreate, k_fold=k_fold)
    ...
    if augment:
        ...
        sub_dirs = split_dataset(dataset_augment, input_file='input.png', data_dir=aug_data_dir, output_dir=aug_ann_dir,
                                 ...
                                 heatmaps=[] if not heatmap_inds else None, k_fold=k_fold)
        if heatmap_inds:
            # Create heatmaps
        ...
        aug_out_dir = os.path.join(os.path.dirname(out_dir), dataset_augment)
        if not os.path.exists(aug_out_dir) or not any([file.split('.')[-1] == 'tfrecords' for file in os.listdir(aug_out_dir)]) or recreate:
            if not os.path.exists(aug_out_dir):
                os.makedirs(aug_out_dir)
            create_dataset(dataset_augment, type='default', input_dirs=aug_ann_dir, output_dir=aug_out_dir,
                           data_dir=os.path.dirname(aug_data_dir), k_fold=k_fold)
        if overlay:
            # Create the overlay
            overs = [sub_dir for sub_dir in sub_dirs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default='data/',
                        help='Data path to process, where data_source folder exist. (data/)')
    parser.add_argument('--ann_dir', default='data/annotations', help='Path to save text split annotations')
    # Flags
    ...
    parser.add_argument('--k_fold', default=0, type=int, help='Number of folds for cross validation (0 disables k-fold).')
    parser.add_argument('--save_data_dir', default='data/tfrecords', help='Directory to save the TFRecords.')
    ...
    ann_dir = os.path.abspath(os.path.join(args.ann_dir, args.data_source))
    out_dir = os.path.abspath(os.path.join(args.save_data_dir, args.data_source))
    ...
    dataset_create(args.dataset_name, args.classes, args.heatmap_inds, args.data_source, dat_dir, ann_dir, out_dir,
                   args.r, args.a, args.o, args.k_fold)
