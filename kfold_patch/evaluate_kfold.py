...
def find_fold_model_dirs(model_prefix, k_fold=10):
    """
    ...
    """
    # Find all parent dirs matching this model prefix, sorted by timestamp (newest last)
    all_run_dirs = sorted(glob.glob(os.path.join(MODELS_DIR, f'{model_prefix}_*')))
...
def evaluate_fold(fold_dir, model_cfg):
    """Evaluate a single fold model on the test set. Returns the confusion matrix."""
    ...
    test_dataset = floorplans.load_test_data(
        classes,
    ...
    for img, mask in test_dataset.batch(1):
        y_pred = model(img, training=False)
        # Handle tuple outputs (e.g. models that return (seg, heatmap))
        if isinstance(y_pred, (tuple, list)):
            y_pred = y_pred[0]
        y_true_idx = tf.math.argmax(mask[0], axis=-1).numpy().flatten()
        y_pred_idx = tf.math.argmax(y_pred[0], axis=-1).numpy().flatten()
...
def print_and_save_results(cm_agg, fold_accs, model_prefix, model_cfg):
    classes = model_cfg['classes']
    ...
    print(f"\n  Aggregated test set results ({model_prefix}, {len(fold_accs)} folds):")
    print(f"  Per-fold accuracy: {[f'{a:.4f}' for a in fold_accs]}")
    print(f"  Mean test accuracy: {np.mean(fold_accs):.4f} ± {np.std(fold_accs):.4f}")
    print(tabulate(table, headers=headers, floatfmt=".4f"))

    os.makedirs(RESULTS_DIR, exist_ok=True)
    timestr = time.strftime("%Y%m%d-%H%M%S")
    results_path = os.path.join(RESULTS_DIR, f'test_kfold_{model_prefix}_{timestr}.txt')
    with open(results_path, 'w') as f:
        ...
        f.write(f"Mean test accuracy: {np.mean(fold_accs):.4f} +/- {np.std(fold_accs):.4f}\n\n")
        f.write(tabulate(table, headers=headers, floatfmt=".4f"))
    print(f"  Results saved to: {results_path}")
    return results_path


def evaluate_model(model_prefix, k_fold=10):
    print(f"\n{'='*60}")
    ...
    fold_dirs = find_fold_model_dirs(model_prefix, k_fold)

    if not fold_dirs:
        print(f"  ERROR: No saved fold models found for prefix '{model_prefix}'")
        return

    print(f"  Found {len(fold_dirs)} fold models: folds {sorted(fold_dirs.keys())}")
    ...
    for fold_idx in sorted(fold_dirs.keys()):
        print(f"\n  Fold {fold_idx}:")
        tic = time.time()
        cm = evaluate_fold(fold_dirs[fold_idx], model_cfg)
        toc = time.time()
    ...
    print_and_save_results(cm_agg, fold_accs, model_prefix, model_cfg)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='K-Fold test set evaluation')
    ...
    for model_prefix in args.models:
        evaluate_model(model_prefix, args.k_fold)
